...
pub struct GetObjectReplicationPolicyResult {
    ...
    pub metrics: Option<storage::v20250601::ObjectReplicationPolicyPropertiesMetricsResponse>,
    /// The name of the resource
    pub name: String,
    /// A unique id for object replication policy.
    pub policy_id: String,
    /// Optional. The object replication policy priority replication feature options.
    pub priority_replication: Option<storage::v20250601::ObjectReplicationPolicyPropertiesPriorityReplicationResponse>,
    /// The storage account object replication rules.
    pub rules: Option<Vec<storage::v20250601::ObjectReplicationPolicyRuleResponse>>,
...
pub async fn get_object_replication_policy(
    ...
    let result = ctx.invoke("azure-native:storage/v20250601:getObjectReplicationPolicy", invoke_args, &opts).await?;
    ...
    Ok(GetObjectReplicationPolicyResult {
        ...
        id: serde_json::from_value(result.fields.get("id").cloned().unwrap_or_default())?
            ,
        metrics: result.fields.get("metrics").cloned().map(serde_json::from_value).transpose()?,
        name: serde_json::from_value(result.fields.get("name").cloned().unwrap_or_default())?
            ,
        policy_id: serde_json::from_value(result.fields.get("policyId").cloned().unwrap_or_default())?
            ,
        priority_replication: result.fields.get("priorityReplication").cloned().map(serde_json::from_value).transpose()?,
