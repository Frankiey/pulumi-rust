...
pub struct QueueServicePropertiesArgs {
    ...
    pub queue_service_name: Option<Input<String>>,
...
impl QueueServiceProperties {
    const TYPE_TOKEN: &'static str = "azure-native:storage/v20250101:QueueServiceProperties";

    /// Create a new QueueServiceProperties resource.
    pub async fn new(
        ctx: &Context,
        name: &str,
        ...
        if let Some(v) = args.queue_service_name {
            pulumi_sdk::resolve_input("queueServiceName", v, &mut inputs, &mut deps, &mut prop_deps).await;
        }
