...
pub struct IpamPoolArgs {
    /// The geo-location where the resource lives
    pub location: Option<Input<String>>,
    /// The name of the network manager.
    pub network_manager_name: Input<String>,
    /// IP Address Manager Pool resource name.
    pub pool_name: Option<Input<String>>,
    /// Properties of IpamPool resource properties which are specific to the Pool resource.
    pub properties: Input<network::v20240101preview::IpamPoolPropertiesArgs>,
...
impl IpamPool {
    const TYPE_TOKEN: &'static str = "azure-native:network/v20240101preview:IpamPool";

    /// Create a new IpamPool resource.
    pub async fn new(
        ctx: &Context,
        name: &str,
        args: IpamPoolArgs,
        ...
        if let Some(v) = args.pool_name {
            pulumi_sdk::resolve_input("poolName", v, &mut inputs, &mut deps, &mut prop_deps).await;
        }
