...
pub struct GetSecurityAdminConfigurationResult {
    ...
    pub network_group_address_space_aggregation_option: Option<String>,
...
pub async fn get_security_admin_configuration(
    ...
    let result = ctx.invoke("azure-native:network/v20240101preview:getSecurityAdminConfiguration", invoke_args, &opts).await?;
    ...
    Ok(GetSecurityAdminConfigurationResult {
        ...
        name: serde_json::from_value(result.fields.get("name").cloned().unwrap_or_default())?
            ,
        network_group_address_space_aggregation_option: result.fields.get("networkGroupAddressSpaceAggregationOption").cloned().map(serde_json::from_value).transpose()?,
