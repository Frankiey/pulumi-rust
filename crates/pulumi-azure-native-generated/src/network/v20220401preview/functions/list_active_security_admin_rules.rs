...
#[derive(Default)]
pub struct ListActiveSecurityAdminRulesArgs {
...
pub struct ListActiveSecurityAdminRulesResult {
    /// When present, the value can be passed to a subsequent query call (together with the same query and scopes used in the current request) to retrieve the next page of data.
    pub skip_token: Option<String>,
...
/// Lists active security admin rules in a network manager.
pub async fn list_active_security_admin_rules(
    ctx: &Context,
    args: ListActiveSecurityAdminRulesArgs,
    opts: Option<InvokeOptions>,
) -> Result<ListActiveSecurityAdminRulesResult> {
    ...
    let result = ctx.invoke("azure-native:network/v20220401preview:listActiveSecurityAdminRules", invoke_args, &opts).await?;

    Ok(ListActiveSecurityAdminRulesResult {
