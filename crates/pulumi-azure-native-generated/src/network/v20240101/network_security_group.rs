...
pub struct NetworkSecurityGroupArgs {
    ...
    pub flush_connection: Option<Input<bool>>,
    /// Resource ID.
    ...
    pub security_rules: Option<Vec<Input<network::v20240101::SecurityRuleArgs>>>,
    /// Resource tags.
...
pub struct NetworkSecurityGroup {
    ...
    /// When enabled, flows created from Network Security Group connections will be re-evaluated when rules are updates. Initial enablement will trigger re-evaluation.
    pub flush_connection: Output<serde_json::Value>,
    /// Resource location.
...
impl NetworkSecurityGroup {
    const TYPE_TOKEN: &'static str = "azure-native:network/v20240101:NetworkSecurityGroup";
    ...
    pub async fn new(
        ...
        if let Some(v) = args.flush_connection {
            pulumi_sdk::resolve_input("flushConnection", v, &mut inputs, &mut deps, &mut prop_deps).await;
        }
        ...
        Ok(Self {
            ...
            flow_logs: registered.outputs.get("flowLogs")
                .cloned()
                .unwrap_or_else(|| Output::unknown(vec![registered.urn.clone()])),
            flush_connection: registered.outputs.get("flushConnection")
                .cloned()
                .unwrap_or_else(|| Output::unknown(vec![registered.urn.clone()])),
