...
pub struct NetworkManagerArgs {
    ...
    /// Scope Access.
    pub network_manager_scope_accesses: Option<Vec<Input<serde_json::Value>>>,
    /// Scope of Network Manager.
    pub network_manager_scopes: Option<Input<network::v20210201preview::NetworkManagerPropertiesNetworkManagerScopesArgs>>,
...
impl NetworkManager {
    const TYPE_TOKEN: &'static str = "azure-native:network/v20210201preview:NetworkManager";
    ...
    pub async fn new(
        ...
        if let Some(v) = args.network_manager_scope_accesses {
            pulumi_sdk::resolve_input_vec("networkManagerScopeAccesses", v, &mut inputs, &mut deps, &mut prop_deps).await;
        }
        if let Some(v) = args.network_manager_scopes {
            pulumi_sdk::resolve_input("networkManagerScopes", v, &mut inputs, &mut deps, &mut prop_deps).await;
        }
