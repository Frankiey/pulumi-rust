...
pub struct SecurityUserConfigurationArgs {
    /// The name of the network manager security Configuration.
    pub configuration_name: Option<Input<String>>,
    /// Flag if need to delete existing network security groups.
    pub delete_existing_ns_gs: Option<Input<serde_json::Value>>,
    ...
    /// Security Type.
    pub security_type: Option<Input<serde_json::Value>>,
}

/// Defines the security configuration
pub struct SecurityUserConfiguration {
    ...
    /// Flag if need to delete existing network security groups.
    pub delete_existing_ns_gs: Output<serde_json::Value>,
    /// A description of the security configuration.
    ...
    /// Security Type.
    pub security_type: Output<serde_json::Value>,
...
impl SecurityUserConfiguration {
    const TYPE_TOKEN: &'static str = "azure-native:network/v20210201preview:SecurityUserConfiguration";

    /// Create a new SecurityUserConfiguration resource.
    pub async fn new(
        ctx: &Context,
        name: &str,
        ...
        if let Some(v) = args.delete_existing_ns_gs {
            pulumi_sdk::resolve_input("deleteExistingNSGs", v, &mut inputs, &mut deps, &mut prop_deps).await;
        }
        ...
        if let Some(v) = args.security_type {
            pulumi_sdk::resolve_input("securityType", v, &mut inputs, &mut deps, &mut prop_deps).await;
        }
        ...
        Ok(Self {
            ...
            azure_api_version: registered.outputs.get("azureApiVersion")
                .cloned()
                .unwrap_or_else(|| Output::unknown(vec![registered.urn.clone()])),
            delete_existing_ns_gs: registered.outputs.get("deleteExistingNSGs")
                .cloned()
                .unwrap_or_else(|| Output::unknown(vec![registered.urn.clone()])),
            ...
            provisioning_state: registered.outputs.get("provisioningState")
                .cloned()
                .unwrap_or_else(|| Output::unknown(vec![registered.urn.clone()])),
            security_type: registered.outputs.get("securityType")
                .cloned()
                .unwrap_or_else(|| Output::unknown(vec![registered.urn.clone()])),
