...
pub struct UserRuleCollectionArgs {
    /// Groups for configuration
    pub applies_to_groups: Option<Vec<Input<network::v20210201preview::NetworkManagerSecurityGroupItemArgs>>>,
...
impl UserRuleCollection {
    const TYPE_TOKEN: &'static str = "azure-native:network/v20210201preview:UserRuleCollection";

    /// Create a new UserRuleCollection resource.
    pub async fn new(
        ctx: &Context,
        name: &str,
        ...
        if let Some(v) = args.applies_to_groups {
            pulumi_sdk::resolve_input_vec("appliesToGroups", v, &mut inputs, &mut deps, &mut prop_deps).await;
        }
