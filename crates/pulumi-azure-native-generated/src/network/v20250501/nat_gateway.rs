...
pub struct NatGatewayArgs {
    ...
    pub public_ip_addresses: Option<Vec<Input<network::v20250501::SubResourceArgs>>>,
    /// An array of public ip addresses V6 associated with the nat gateway resource.
    pub public_ip_addresses_v6: Option<Vec<Input<network::v20250501::SubResourceArgs>>>,
    /// An array of public ip prefixes V4 associated with the nat gateway resource.
    pub public_ip_prefixes: Option<Vec<Input<network::v20250501::SubResourceArgs>>>,
    /// An array of public ip prefixes V6 associated with the nat gateway resource.
    pub public_ip_prefixes_v6: Option<Vec<Input<network::v20250501::SubResourceArgs>>>,
    /// The name of the resource group.
    pub resource_group_name: Input<String>,
    /// Reference to an existing service gateway.
    pub service_gateway: Option<Input<network::v20250501::SubResourceArgs>>,
    /// The nat gateway SKU.
    pub sku: Option<Input<network::v20250501::NatGatewaySkuArgs>>,
    /// A reference to the source virtual network using this nat gateway resource.
    pub source_virtual_network: Option<Input<network::v20250501::SubResourceArgs>>,
    /// Resource tags.
...
pub struct NatGateway {
    ...
    /// An array of public ip addresses V6 associated with the nat gateway resource.
    pub public_ip_addresses_v6: Output<serde_json::Value>,
    /// An array of public ip prefixes V4 associated with the nat gateway resource.
    pub public_ip_prefixes: Output<serde_json::Value>,
    /// An array of public ip prefixes V6 associated with the nat gateway resource.
    pub public_ip_prefixes_v6: Output<serde_json::Value>,
    /// The resource GUID property of the NAT gateway resource.
    pub resource_guid: Output<serde_json::Value>,
    /// Reference to an existing service gateway.
    pub service_gateway: Output<serde_json::Value>,
    /// The nat gateway SKU.
    pub sku: Output<serde_json::Value>,
    /// A reference to the source virtual network using this nat gateway resource.
    pub source_virtual_network: Output<serde_json::Value>,
...
impl NatGateway {
    const TYPE_TOKEN: &'static str = "azure-native:network/v20250501:NatGateway";
    ...
    pub async fn new(
        ...
        if let Some(v) = args.public_ip_addresses_v6 {
            pulumi_sdk::resolve_input_vec("publicIpAddressesV6", v, &mut inputs, &mut deps, &mut prop_deps).await;
        }
        ...
        if let Some(v) = args.public_ip_prefixes_v6 {
            pulumi_sdk::resolve_input_vec("publicIpPrefixesV6", v, &mut inputs, &mut deps, &mut prop_deps).await;
        }
        pulumi_sdk::resolve_input("resourceGroupName", args.resource_group_name, &mut inputs, &mut deps, &mut prop_deps).await;
        if let Some(v) = args.service_gateway {
            pulumi_sdk::resolve_input("serviceGateway", v, &mut inputs, &mut deps, &mut prop_deps).await;
        }
        if let Some(v) = args.sku {
            pulumi_sdk::resolve_input("sku", v, &mut inputs, &mut deps, &mut prop_deps).await;
        }
        if let Some(v) = args.source_virtual_network {
            pulumi_sdk::resolve_input("sourceVirtualNetwork", v, &mut inputs, &mut deps, &mut prop_deps).await;
        }
        ...
        Ok(Self {
            ...
            public_ip_addresses: registered.outputs.get("publicIpAddresses")
                .cloned()
                .unwrap_or_else(|| Output::unknown(vec![registered.urn.clone()])),
            public_ip_addresses_v6: registered.outputs.get("publicIpAddressesV6")
                .cloned()
                .unwrap_or_else(|| Output::unknown(vec![registered.urn.clone()])),
            public_ip_prefixes: registered.outputs.get("publicIpPrefixes")
                .cloned()
                .unwrap_or_else(|| Output::unknown(vec![registered.urn.clone()])),
            public_ip_prefixes_v6: registered.outputs.get("publicIpPrefixesV6")
                .cloned()
                .unwrap_or_else(|| Output::unknown(vec![registered.urn.clone()])),
            resource_guid: registered.outputs.get("resourceGuid")
                .cloned()
                .unwrap_or_else(|| Output::unknown(vec![registered.urn.clone()])),
            service_gateway: registered.outputs.get("serviceGateway")
                .cloned()
                .unwrap_or_else(|| Output::unknown(vec![registered.urn.clone()])),
            sku: registered.outputs.get("sku")
                .cloned()
                .unwrap_or_else(|| Output::unknown(vec![registered.urn.clone()])),
            source_virtual_network: registered.outputs.get("sourceVirtualNetwork")
                .cloned()
                .unwrap_or_else(|| Output::unknown(vec![registered.urn.clone()])),
