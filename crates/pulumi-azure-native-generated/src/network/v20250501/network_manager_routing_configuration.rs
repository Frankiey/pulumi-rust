...
pub struct NetworkManagerRoutingConfigurationArgs {
    ...
    pub route_table_usage_mode: Option<Input<serde_json::Value>>,
}

/// Defines the routing configuration
pub struct NetworkManagerRoutingConfiguration {
    ...
    /// Route table usage mode defines which route table will be used by the configuration. If not defined, this will default to 'ManagedOnly'.
    pub route_table_usage_mode: Output<serde_json::Value>,
...
impl NetworkManagerRoutingConfiguration {
    const TYPE_TOKEN: &'static str = "azure-native:network/v20250501:NetworkManagerRoutingConfiguration";

    /// Create a new NetworkManagerRoutingConfiguration resource.
    pub async fn new(
        ctx: &Context,
        name: &str,
        ...
        if let Some(v) = args.route_table_usage_mode {
            pulumi_sdk::resolve_input("routeTableUsageMode", v, &mut inputs, &mut deps, &mut prop_deps).await;
        }
        ...
        Ok(Self {
            ...
            resource_guid: registered.outputs.get("resourceGuid")
                .cloned()
                .unwrap_or_else(|| Output::unknown(vec![registered.urn.clone()])),
            route_table_usage_mode: registered.outputs.get("routeTableUsageMode")
                .cloned()
                .unwrap_or_else(|| Output::unknown(vec![registered.urn.clone()])),
