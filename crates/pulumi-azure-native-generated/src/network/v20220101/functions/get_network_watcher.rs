...
pub struct GetNetworkWatcherResult {
    ...
    pub running_operation_ids: Option<Vec<String>>,
    /// Resource tags.
...
pub async fn get_network_watcher(
    ...
    let result = ctx.invoke("azure-native:network/v20220101:getNetworkWatcher", invoke_args, &opts).await?;
    ...
    Ok(GetNetworkWatcherResult {
        ...
        provisioning_state: serde_json::from_value(result.fields.get("provisioningState").cloned().unwrap_or_default())?
            ,
        running_operation_ids: result.fields.get("runningOperationIds").cloned().map(serde_json::from_value).transpose()?,
