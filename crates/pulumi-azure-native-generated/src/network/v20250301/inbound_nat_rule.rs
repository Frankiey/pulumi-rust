...
pub struct InboundNatRuleArgs {
    ...
    pub backend_address_pool: Option<Input<network::v20250301::SubResourceArgs>>,
...
pub struct InboundNatRule {
    ...
    /// A reference to backendAddressPool resource.
    pub backend_address_pool: Output<serde_json::Value>,
    ...
    /// The port range end for the external endpoint. This property is used together with BackendAddressPool and FrontendPortRangeStart. Individual inbound NAT rule port mappings will be created for each backend address from BackendAddressPool. Acceptable values range from 1 to 65534.
    pub frontend_port_range_end: Output<serde_json::Value>,
    /// The port range start for the external endpoint. This property is used together with BackendAddressPool and FrontendPortRangeEnd. Individual inbound NAT rule port mappings will be created for each backend address from BackendAddressPool. Acceptable values range from 1 to 65534.
    pub frontend_port_range_start: Output<serde_json::Value>,
    /// The timeout for the TCP idle connection. The value can be set between 4 and 30 minutes. The default value is 4 minutes. This element is only used when the protocol is set to TCP.
...
impl InboundNatRule {
    const TYPE_TOKEN: &'static str = "azure-native:network/v20250301:InboundNatRule";
    ...
    pub async fn new(
        ...
        if let Some(v) = args.backend_address_pool {
            pulumi_sdk::resolve_input("backendAddressPool", v, &mut inputs, &mut deps, &mut prop_deps).await;
        }
        ...
        if let Some(v) = args.frontend_port_range_end {
            pulumi_sdk::resolve_input("frontendPortRangeEnd", v, &mut inputs, &mut deps, &mut prop_deps).await;
        }
        if let Some(v) = args.frontend_port_range_start {
            pulumi_sdk::resolve_input("frontendPortRangeStart", v, &mut inputs, &mut deps, &mut prop_deps).await;
        }
        ...
        Ok(Self {
            ...
            azure_api_version: registered.outputs.get("azureApiVersion")
                .cloned()
                .unwrap_or_else(|| Output::unknown(vec![registered.urn.clone()])),
            backend_address_pool: registered.outputs.get("backendAddressPool")
                .cloned()
                .unwrap_or_else(|| Output::unknown(vec![registered.urn.clone()])),
            ...
            frontend_port: registered.outputs.get("frontendPort")
                .cloned()
                .unwrap_or_else(|| Output::unknown(vec![registered.urn.clone()])),
            frontend_port_range_end: registered.outputs.get("frontendPortRangeEnd")
                .cloned()
                .unwrap_or_else(|| Output::unknown(vec![registered.urn.clone()])),
            frontend_port_range_start: registered.outputs.get("frontendPortRangeStart")
                .cloned()
                .unwrap_or_else(|| Output::unknown(vec![registered.urn.clone()])),
