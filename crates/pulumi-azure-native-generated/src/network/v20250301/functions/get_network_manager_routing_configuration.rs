...
pub struct GetNetworkManagerRoutingConfigurationResult {
    ...
    pub route_table_usage_mode: Option<String>,
...
pub async fn get_network_manager_routing_configuration(
    ...
    let result = ctx.invoke("azure-native:network/v20250301:getNetworkManagerRoutingConfiguration", invoke_args, &opts).await?;
    ...
    Ok(GetNetworkManagerRoutingConfigurationResult {
        ...
        resource_guid: serde_json::from_value(result.fields.get("resourceGuid").cloned().unwrap_or_default())?
            ,
        route_table_usage_mode: result.fields.get("routeTableUsageMode").cloned().map(serde_json::from_value).transpose()?,
