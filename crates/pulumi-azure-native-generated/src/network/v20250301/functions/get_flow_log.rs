...
pub struct GetFlowLogResult {
    ...
    pub enabled_filtering_criteria: Option<String>,
    /// A unique read-only string that changes whenever the resource is updated.
    pub etag: String,
    /// Parameters that define the configuration of traffic analytics.
    pub flow_analytics_configuration: Option<network::v20250301::TrafficAnalyticsPropertiesResponse>,
    /// Parameters that define the flow log format.
    pub format: Option<network::v20250301::FlowLogFormatParametersResponse>,
    /// Resource ID.
    ...
    pub retention_policy: Option<network::v20250301::RetentionPolicyParametersResponse>,
...
pub async fn get_flow_log(
    ...
    let result = ctx.invoke("azure-native:network/v20250301:getFlowLog", invoke_args, &opts).await?;

    Ok(GetFlowLogResult {
        azure_api_version: serde_json::from_value(result.fields.get("azureApiVersion").cloned().unwrap_or_default())?
            ,
        enabled: result.fields.get("enabled").cloned().map(serde_json::from_value).transpose()?,
        enabled_filtering_criteria: result.fields.get("enabledFilteringCriteria").cloned().map(serde_json::from_value).transpose()?,
        ...
        provisioning_state: serde_json::from_value(result.fields.get("provisioningState").cloned().unwrap_or_default())?
            ,
        record_types: result.fields.get("recordTypes").cloned().map(serde_json::from_value).transpose()?,
