...
pub struct RoutingRuleCollectionArgs {
    ...
    pub applies_to: Vec<Input<network::v20240901preview::NetworkManagerRoutingGroupItemArgs>>,
    ...
    pub disable_bgp_route_propagation: Option<Input<serde_json::Value>>,
    /// The name of the network manager.
    pub network_manager_name: Input<String>,
    /// Rule to define peering route propagation behavior.
    pub peering_route_propagation_rules: Option<Vec<Input<network::v20240901preview::PeeringRoutePropagationRuleArgs>>>,
...
pub struct RoutingRuleCollection {
    ...
    /// Rule to define peering route propagation behavior.
    pub peering_route_propagation_rules: Output<serde_json::Value>,
...
impl RoutingRuleCollection {
    const TYPE_TOKEN: &'static str = "azure-native:network/v20240901preview:RoutingRuleCollection";
    ...
    pub async fn new(
        ...
        if let Some(v) = args.peering_route_propagation_rules {
            pulumi_sdk::resolve_input_vec("peeringRoutePropagationRules", v, &mut inputs, &mut deps, &mut prop_deps).await;
        }
        ...
        Ok(Self {
            ...
            name: registered.outputs.get("name")
                .cloned()
                .unwrap_or_else(|| Output::unknown(vec![registered.urn.clone()])),
            peering_route_propagation_rules: registered.outputs.get("peeringRoutePropagationRules")
                .cloned()
                .unwrap_or_else(|| Output::unknown(vec![registered.urn.clone()])),
