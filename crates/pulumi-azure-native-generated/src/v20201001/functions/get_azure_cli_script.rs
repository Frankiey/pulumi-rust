...
#[derive(Default)]
pub struct GetAzureCliScriptArgs {
...
pub struct GetAzureCliScriptResult {
    ...
    /// Container settings.
    pub container_settings: Option<resources::v20201001::ContainerConfigurationResponse>,
    /// The environment variables to pass over to the script.
    pub environment_variables: Option<Vec<resources::v20201001::EnvironmentVariableResponse>>,
    /// Gets or sets how the deployment script should be forced to execute even if the script resource has not changed. Can be current time stamp or a GUID.
    pub force_update_tag: Option<String>,
    /// String Id used to locate any resource on Azure.
    pub id: String,
    /// Optional property. Managed identity to be used for this deployment script. Currently, only user-assigned MSI is supported.
    pub identity: Option<resources::v20201001::ManagedServiceIdentityResponse>,
    /// Type of the script.
    pub kind: String,
    ...
    pub status: resources::v20201001::ScriptStatusResponse,
    /// Storage Account settings.
    pub storage_account_settings: Option<resources::v20201001::StorageAccountConfigurationResponse>,
    /// Supporting files for the external script.
    pub supporting_script_uris: Option<Vec<String>>,
    /// The system metadata related to this resource.
    pub system_data: resources::v20201001::SystemDataResponse,
    /// Resource tags.
...
/// Gets a deployment script with a given name.
pub async fn get_azure_cli_script(
    ctx: &Context,
    args: GetAzureCliScriptArgs,
    opts: Option<InvokeOptions>,
) -> Result<GetAzureCliScriptResult> {
    let mut invoke_args = HashMap::new();
    invoke_args.insert("resourceGroupName".to_string(), json!(args.resource_group_name));
    invoke_args.insert("scriptName".to_string(), json!(args.script_name));

    let opts = opts.unwrap_or_default();
    let result = ctx.invoke("azure-native:resources/v20201001:getAzureCliScript", invoke_args, &opts).await?;

    Ok(GetAzureCliScriptResult {
        arguments: result.fields.get("arguments").cloned().map(serde_json::from_value).transpose()?,
        az_cli_version: serde_json::from_value(result.fields.get("azCliVersion").cloned().unwrap_or_default())?
            ,
