...
pub struct GetVpnConnectionResult {
    ...
    pub connection_bandwidth_in_mbps: i64,
    ...
    pub remote_vpn_site: Option<network::v20180601::SubResourceResponse>,
...
pub async fn get_vpn_connection(
    ...
    let result = ctx.invoke("azure-native:network/v20180601:getVpnConnection", invoke_args, &opts).await?;

    Ok(GetVpnConnectionResult {
        azure_api_version: serde_json::from_value(result.fields.get("azureApiVersion").cloned().unwrap_or_default())?
            ,
        connection_bandwidth_in_mbps: serde_json::from_value(result.fields.get("connectionBandwidthInMbps").cloned().unwrap_or_default())?
            ,
