...
pub struct LoadBalancerArgs {
    ...
    pub backend_address_pools: Option<Vec<Input<network::v20180601::BackendAddressPoolArgs>>>,
    /// Object representing the frontend IPs to be used for the load balancer
    pub frontend_ip_configurations: Option<Vec<Input<network::v20180601::FrontendIPConfigurationArgs>>>,
    /// Resource ID.
    pub id: Option<Input<String>>,
    /// Defines an external port range for inbound NAT to a single backend port on NICs associated with a load balancer. Inbound NAT rules are created automatically for each NIC associated with the Load Balancer using an external port from this range. Defining an Inbound NAT pool on your Load Balancer is mutually exclusive with defining inbound Nat rules. Inbound NAT pools are referenced from virtual machine scale sets. NICs that are associated with individual virtual machines cannot reference an inbound NAT pool. They have to reference individual inbound NAT rules.
    pub inbound_nat_pools: Option<Vec<Input<network::v20180601::InboundNatPoolArgs>>>,
    /// Collection of inbound NAT Rules used by a load balancer. Defining inbound NAT rules on your load balancer is mutually exclusive with defining an inbound NAT pool. Inbound NAT pools are referenced from virtual machine scale sets. NICs that are associated with individual virtual machines cannot reference an Inbound NAT pool. They have to reference individual inbound NAT rules.
    pub inbound_nat_rules: Option<Vec<Input<network::v20180601::InboundNatRuleArgs>>>,
    /// The name of the load balancer.
    pub load_balancer_name: Option<Input<String>>,
    /// Object collection representing the load balancing rules Gets the provisioning 
    pub load_balancing_rules: Option<Vec<Input<network::v20180601::LoadBalancingRuleArgs>>>,
    /// Resource location.
    pub location: Option<Input<String>>,
    /// The outbound NAT rules.
    pub outbound_nat_rules: Option<Vec<Input<network::v20180601::OutboundNatRuleArgs>>>,
    /// Collection of probe objects used in the load balancer
    pub probes: Option<Vec<Input<network::v20180601::ProbeArgs>>>,
    ...
    pub sku: Option<Input<network::v20180601::LoadBalancerSkuArgs>>,
    /// Resource tags.
...
pub struct LoadBalancer {
    ...
    /// The outbound NAT rules.
    pub outbound_nat_rules: Output<serde_json::Value>,
    /// Collection of probe objects used in the load balancer
...
impl LoadBalancer {
    const TYPE_TOKEN: &'static str = "azure-native:network/v20180601:LoadBalancer";
    ...
    pub async fn new(
        ...
        if let Some(v) = args.outbound_nat_rules {
            pulumi_sdk::resolve_input_vec("outboundNatRules", v, &mut inputs, &mut deps, &mut prop_deps).await;
        }
        ...
        Ok(Self {
            ...
            name: registered.outputs.get("name")
                .cloned()
                .unwrap_or_else(|| Output::unknown(vec![registered.urn.clone()])),
            outbound_nat_rules: registered.outputs.get("outboundNatRules")
                .cloned()
                .unwrap_or_else(|| Output::unknown(vec![registered.urn.clone()])),
