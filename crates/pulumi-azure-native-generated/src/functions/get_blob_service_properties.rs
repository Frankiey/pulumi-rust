...
pub struct GetBlobServicePropertiesResult {
    ...
    pub change_feed: Option<storage::ChangeFeedResponse>,
    /// The blob service properties for container soft delete.
    pub container_delete_retention_policy: Option<storage::DeleteRetentionPolicyResponse>,
    /// Specifies CORS rules for the Blob service. You can include up to five CorsRule elements in the request. If no CorsRule elements are included in the request body, all CORS rules will be deleted, and CORS will be disabled for the Blob service.
    pub cors: Option<storage::CorsRulesResponse>,
    /// DefaultServiceVersion indicates the default version to use for requests to the Blob service if an incoming request’s version is not specified. Possible values include version 2008-10-27 and all more recent versions.
    pub default_service_version: Option<String>,
    /// The blob service properties for blob soft delete.
    pub delete_retention_policy: Option<storage::DeleteRetentionPolicyResponse>,
    /// Fully qualified resource ID for the resource. Ex - /subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/{resourceProviderNamespace}/{resourceType}/{resourceName}
    pub id: String,
    /// Versioning is enabled if set to true.
    pub is_versioning_enabled: Option<bool>,
    /// The blob service property to configure last access time based tracking policy.
    pub last_access_time_tracking_policy: Option<storage::LastAccessTimeTrackingPolicyResponse>,
    /// The name of the resource
    pub name: String,
    /// The blob service properties for blob restore policy.
    pub restore_policy: Option<storage::RestorePolicyPropertiesResponse>,
    /// Sku name and tier.
...
pub async fn get_blob_service_properties(
    ...
    let result = ctx.invoke("azure-native:storage:getBlobServiceProperties", invoke_args, &opts).await?;
