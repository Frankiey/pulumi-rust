...
#[derive(Default)]
pub struct GetUserRuleCollectionArgs {
...
pub struct GetUserRuleCollectionResult {
    /// Groups for configuration
    pub applies_to_groups: Option<Vec<network::v20210201preview::NetworkManagerSecurityGroupItemResponse>>,
...
/// Gets a network manager security user configuration rule collection.
pub async fn get_user_rule_collection(
    ctx: &Context,
    args: GetUserRuleCollectionArgs,
    opts: Option<InvokeOptions>,
) -> Result<GetUserRuleCollectionResult> {
    ...
    let result = ctx.invoke("azure-native:network/v20210201preview:getUserRuleCollection", invoke_args, &opts).await?;

    Ok(GetUserRuleCollectionResult {
        applies_to_groups: result.fields.get("appliesToGroups").cloned().map(serde_json::from_value).transpose()?,
