...
pub async fn get_security_admin_configuration(
    ...
    let result = ctx.invoke("azure-native:network/v20210201preview:getSecurityAdminConfiguration", invoke_args, &opts).await?;
    ...
    Ok(GetSecurityAdminConfigurationResult {
        ...
        provisioning_state: serde_json::from_value(result.fields.get("provisioningState").cloned().unwrap_or_default())?
            ,
        security_type: result.fields.get("securityType").cloned().map(serde_json::from_value).transpose()?,
