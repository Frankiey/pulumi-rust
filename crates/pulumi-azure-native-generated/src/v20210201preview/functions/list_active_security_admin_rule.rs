...
#[derive(Default)]
pub struct ListActiveSecurityAdminRuleArgs {
...
pub struct ListActiveSecurityAdminRuleResult {
    /// When present, the value can be passed to a subsequent query call (together with the same query and scopes used in the current request) to retrieve the next page of data.
    pub skip_token: Option<String>,
...
/// Lists active security admin rules in a network manager.
pub async fn list_active_security_admin_rule(
    ctx: &Context,
    args: ListActiveSecurityAdminRuleArgs,
    opts: Option<InvokeOptions>,
) -> Result<ListActiveSecurityAdminRuleResult> {
    ...
    let result = ctx.invoke("azure-native:network/v20210201preview:listActiveSecurityAdminRule", invoke_args, &opts).await?;

    Ok(ListActiveSecurityAdminRuleResult {
