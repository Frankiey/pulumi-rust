...
#[derive(Default)]
pub struct ListActiveSecurityUserRuleArgs {
...
pub struct ListActiveSecurityUserRuleResult {
    /// When present, the value can be passed to a subsequent query call (together with the same query and scopes used in the current request) to retrieve the next page of data.
    pub skip_token: Option<String>,
...
/// Lists Active Security User Rules in a network manager.
pub async fn list_active_security_user_rule(
    ctx: &Context,
    args: ListActiveSecurityUserRuleArgs,
    opts: Option<InvokeOptions>,
) -> Result<ListActiveSecurityUserRuleResult> {
    ...
    let result = ctx.invoke("azure-native:network/v20210201preview:listActiveSecurityUserRule", invoke_args, &opts).await?;

    Ok(ListActiveSecurityUserRuleResult {
