...
#[derive(Default)]
pub struct GetDeploymentStackAtSubscriptionArgs {
    /// Name of the deployment stack.
    pub deployment_stack_name: String,
}

/// Result of the function invocation.
pub struct GetDeploymentStackAtSubscriptionResult {
...
/// Gets the Deployment stack with the given name.
pub async fn get_deployment_stack_at_subscription(
    ctx: &Context,
    args: GetDeploymentStackAtSubscriptionArgs,
    opts: Option<InvokeOptions>,
) -> Result<GetDeploymentStackAtSubscriptionResult> {
    let mut invoke_args = HashMap::new();
    invoke_args.insert("deploymentStackName".to_string(), json!(args.deployment_stack_name));

    let opts = opts.unwrap_or_default();
    let result = ctx.invoke("azure-native:resources/v20250701:getDeploymentStackAtSubscription", invoke_args, &opts).await?;

    Ok(GetDeploymentStackAtSubscriptionResult {
